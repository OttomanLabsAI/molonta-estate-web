...
| v1.16 | The newsletter, and where the code lives | The page now asks, once and politely, for an email address — a quiet sign-up band before the footer and a small call that appears once a visit, both reaching the family directly. And the offer now names GitHub as the home of the code the estate owns. |
| v1.17 | Before, after, and everything gained between | The demo now opens on the estate's current site before showing the new one — and the offer lists all the demo has gained since its first showing, from the grand-hotel structure and the gallery to the newsletter and the remade brochure, inside the same two numbers. |
| v1.18 | A finder's fee, in writing | The offer now says out loud what passing the page on is worth — fifty pounds a referred purchase, seventy-five once you're a customer yourself — set just before the closing word. |
| v1.19 | The demo learns to introduce itself | Shared anywhere — a message, a mail, a feed — the link now unfurls into a proper card: the estate's own sunset over the bay with the wordmark set in cream across it. |
